...
import { saveNewOrder } from '~/services/orderServices';
                            ...
                            <PayPalButton
                                ...
                                onApprove={(data, actions) => {
                                    return actions.order.capture().then(function (details) {
                                        const load = saveNewOrder(data?.orderID, 1);
                                        load.then((e) => {
                                            if (e?.status === 200) {
                                                navigate('/');
                                            } else {
                            ...
                            <div className="rounded-lg bg-[#161616] p-4">
                                ...
                                <div>
                                    <p className="my-4 text-center text-[14px]">
                                        Bằng việc chấp nhận thanh toán, Khách Hàng đã đồng ý với Điều kiện và điều khoản
                                        sử dụng dịch vụ truyền hình FPT Play, và ủy quyền cho FPT Play tự động gia hạn
                                        khi hết hạn sử dụng
                                    </p>
                                </div>
                            </div>
